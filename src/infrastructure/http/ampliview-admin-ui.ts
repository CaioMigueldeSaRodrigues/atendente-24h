...
export function renderAmpliviewAdminUi(data: AmpliviewAdminDemoData, marketMapping: AmpliviewAdminMarketData): string {
  const safeData = JSON.stringify(data)
    ...
    .replace(/\u2028/g, "\\u2028")
    .replace(/\u2029/g, "\\u2029");
  const safeMarketMapping = JSON.stringify(marketMapping)
    .replace(/</g, "\\u003c")
    .replace(/\u2028/g, "\\u2028")
    .replace(/\u2029/g, "\\u2029");
...
function renderMappedMarket(){heading("Mercado Mapeado","Levantamento público do mercado automotivo de Manaus por macrorregião e conglomerado.");const notice=el("aside",undefined,"market-notice");notice.append(el("strong","Empresas encontradas neste levantamento são estabelecimentos do mercado e não representam clientes Ampliview."),el("p","O levantamento é progressivo e não constitui censo completo do setor."));const metrics=el("div",undefined,"metric-grid");for(const entry of [["Macrorregiões",market.regions.length],["Conglomerados definidos",market.clusters.length],["Empresas mapeadas",market.mappedBusinessCount]]){const metric=el("article",undefined,"metric-card");metric.append(el("p",entry[0],"metric-label"),el("strong",Number(entry[1]).toLocaleString("pt-BR"),"metric-value"));metrics.append(metric);}content.append(notice,metrics);if(market.mappedBusinessCount===0)content.append(el("p","Carga de estabelecimentos validada ainda não integrada.","empty-state"));for(const region of market.regions){const clusters=market.clusters.filter(cluster=>cluster.region===region);content.append(el("h2",region,"section-heading"));const list=el("div",undefined,"cluster-grid");for(const cluster of clusters){const item=el("article",undefined,"cluster-card");item.append(el("h3",cluster.name),el("p",cluster.neighborhoods.join(" · ")));list.append(item);}content.append(list);}}
