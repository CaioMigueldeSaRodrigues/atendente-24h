...
export function renderAmpliviewAdminUi(data: AmpliviewAdminDemoData): string {
...
</style></head><body><div class="shell"><aside class="sidebar"><div class="brand"><span class="mark" aria-hidden="true">A</span><span><strong>Ampliview</strong><small>Inteligência comercial e operacional</small></span></div><p class="portal-label">Portal interno</p><nav class="navigation" aria-label="Navegação do painel">${nav}</nav></aside><main class="main"><div class="topline"><p class="portal-title">Portal Ampliview</p><span class="demo-badge">Dados de demonstração</span></div><aside class="demo-notice" role="note"><strong>Dados de demonstração</strong><p>Os indicadores desta tela são exemplos para validação da experiência e ainda não representam dados reais da operação.</p></aside><section id="admin-content" aria-live="polite"></section><p class="footer-note">Ampliview · ambiente de demonstração</p></main></div>
<script>
"use strict";
const demo=${safeData};
...
function renderAdoption(){heading("Adesão e Cobertura","Exemplos demonstrativos da expansão comercial por região.");content.append(table(["Região","Empresas ativas","Novas empresas","Segmentos atendidos","Penetração relativa","Tendência"],demo.regionalAdoption.map(x=>[x.region,x.activeBusinesses,x.newBusinesses,x.segments.join(" / "),x.penetrationPercent+"%",x.trend])));const coverage=el("div",undefined,"two-column");for(const deficit of demo.coverageDeficits){const item=el("article",undefined,"opportunity-card");item.append(el("h3",deficit.area),el("p",deficit.signal),el("p",deficit.coverage),el("span","Exemplo de oportunidade","demo-tag"));coverage.append(item);}content.append(el("h2","Déficits de cobertura","section-heading"),coverage);}
...
function renderMerchandising(){heading("Merchandising","Oportunidades comerciais e tendências demonstrativas.");content.append(el("h2","Demanda não explorada","section-heading"),table(["Produto/serviço","Interesse identificado","Oferta apresentada","Conversão","Gap comercial"],demo.merchandising.unexploredDemand.map(x=>[x.item,x.interests,x.offers,x.conversions,x.gap])));const regions=el("div",undefined,"two-column");for(const item of demo.merchandising.opportunitiesByRegion){const box=el("article",undefined,"opportunity-card");box.append(el("h3",item.region),el("p",item.segment),el("p",item.demand+" · "+item.coverage),el("span","Exemplo de oportunidade","demo-tag"));regions.append(box);}content.append(el("h2","Oportunidades por região","section-heading"),regions,el("h2","Oportunidades por segmento","section-heading"),table(["Segmento","Sinal demonstrativo"],demo.merchandising.opportunitiesBySegment.map(x=>[x.segment,x.signal])),el("h2","Tendências de produtos/serviços","section-heading"),card("Variação demonstrativa",bars(demo.merchandising.productTrends.map(x=>({item:x.item,count:x.changePercent})),"item","count","%")));}
